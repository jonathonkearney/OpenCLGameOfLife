...
class LifeEnv extends Canvas {
    ...
    public LifeEnv() {
        ...
        //square around the edges, initially chosen to see if the algorithm for adding rows/cols was working
        for (int i = 0; i < N; i++) {
            current[0][i] = 1;
            current[99][i] = 1;
            current[i][99] = 1;
    ...
    public void runIterations(){
        ...
        clEnqueueWriteBuffer(commandQueue, memObjects[ 0 ], CL_TRUE, 0, Sizeof.cl_int*(N+2)*(N+2), firstArrayPointer, 0, null, null);
        clEnqueueWriteBuffer(commandQueue, memObjects[ 1 ], CL_TRUE, 0, Sizeof.cl_int*(N+2)*(N+2), secondArrayPointer, 0, null, null);
        ...
        final long calc_global_work_size[] = new long[]{100, 100}; // required because 2D kernels need 2 arguments
        final long calc_local_work_size[] = new long[]{1, 1};

        int iter = 0;
        // this while loop infinitely executes the kernels and switches between two versions of the array.
        // in each loop one array stores the results of the last iteration and the other one stores the results of the current one
        // after the computation we switch the arrays. Effectively switching between Current and Update.
        // looping here saves calling the above initialization code over and over
        while(true) {

            //used to calculate the execution time. The time until now was tested to be around 600ms on my machine.
            long startTime = System.nanoTime();

            //do the calculations on the cells
            clEnqueueNDRangeKernel(commandQueue, k_addSideRows, 1, null, global_work_size, local_work_size, 0, null, null);
            clEnqueueNDRangeKernel(commandQueue, k_addSideCols, 1, null, global_work_size, local_work_size, 0, null, null);
            clEnqueueNDRangeKernel(commandQueue, k_calculate, 2, null, calc_global_work_size, calc_local_work_size, 0, null, null);


            if(iter%2 == 1) {
                //reassign the arguments for the next iteration, swapping the two arrays
                clSetKernelArg(k_addSideRows, 0, Sizeof.cl_mem, Pointer.to(memObjects[0]));
                clSetKernelArg(k_addSideCols, 0, Sizeof.cl_mem, Pointer.to(memObjects[0]));
                clSetKernelArg(k_calculate, 0, Sizeof.cl_mem, Pointer.to(memObjects[0]));
                clSetKernelArg(k_calculate, 1, Sizeof.cl_mem, Pointer.to(memObjects[1]));
                ...
                clEnqueueReadBuffer(commandQueue, memObjects[1], CL_TRUE, 0, Sizeof.cl_int * (N + 2) * (N + 2), secondArrayPointer, 0, null, null);
                ...
                clEnqueueReadBuffer(commandQueue, memObjects[0], CL_TRUE, 0, Sizeof.cl_int * (N + 2) * (N + 2), secondArrayPointer, 0, null, null);
                ...
                //if (iter == 0){ // method to print the execution time of one iteration
